...
int main() {
    ...
    for ( int i = 0; i < numberOfDevices; i++ )
    {
        ...
        std::cout << "\tlibusb_get_bus_number(dev): " << std::hex << (int)libusb_get_bus_number(dev) << std::endl;
...
//                 switch ( libusb_claim_interface(handle, 0) )
//                 {
//                     case 0: // success
//                     {
                        int maxPacketSize = libusb_get_max_packet_size(dev, endpoint);

                        switch ( libusb_bulk_transfer (handle, endpoint, data, length, &transferred, 100) )
                        {
                            case 0: // on success (and populates transferred)
                                std::cout << ">>> SUCCESS, maxPacketSize = " << maxPacketSize << " <<<" << std::endl;
                                break;
