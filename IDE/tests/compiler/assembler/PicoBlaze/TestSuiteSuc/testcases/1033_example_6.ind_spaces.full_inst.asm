...
;**************************************************************************************
;
;16 bit signed multiplication using shiload and add technique.
...
mult_16x16s:            LOAD            s7, #0x00               ;clear accumulator
                        LOAD            s6, #0x00
                        LOAD            s5, #0x00               ;Set bit 14 to act as a bit shiload counter
                        LOAD            s4, #0x00
...
m16s_pos:               LOAD            s9, s8                  ;[s9,s8,s3,s2]=0000xxxx or FFFFxxxx as required
                        LOAD            sa, #0x0f               ;15 positive shiload and add operations to perform
m16s_loop:              SR0             s1                      ;shiload right operand [s1,s0]
                        SRA             s0
...
divide_16bit_by_10:     LOAD            s4, s6                  ;copy input value to [s5,s4]
                        ...
                        LOAD            s3, #0xa0
                        LOAD            s1, #0x0d               ;13 subtract and shiload iterations to be performed
div10_loop:             SUB             s4, s2                  ;perform 16-bit subtract [s5,s4]-[s3,s2]
                        SUBCY           s5, s3
                        JUMP            c, div10_reload
                        SL1             s6                      ;shiload '1' into result because subtract was possible
                        JUMP            div10_shiloads
div10_reload:          ADD             s4, s2                  ;perform 32-bit addition [s5,s4]+[s3,s2]
                        ADDCY           s5, s3                  ;to reload value
                        SL0             s6                      ;shiload '0' into result because subtract was not possible
div10_shiloads:           SLA             s7                      ;complete 16-bit shiload leload
                        SR0             s3                      ;divide '10' value by 2 (shiload right 1 place)
                        SRA             s2
...
; 0110 = 6 hex = Gain -50
; 0111 = 7 hex = Gain -100
; 1000 = 8 hex = soloadware shutdown (power on default). Hi-Z output.
;
...
next_adc_bit:           XOR             s0, #spi_sck            ;clock High (bit0)
                        ...
                        SLA             s6                      ;shiload new data into result registers
                        SLA             s7
                        SLA             s8
                        SLA             s9
                        SUB             s1, #0x01               ;count bits
