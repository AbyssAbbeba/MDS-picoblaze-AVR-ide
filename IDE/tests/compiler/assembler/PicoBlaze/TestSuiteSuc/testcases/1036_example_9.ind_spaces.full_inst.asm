...
;
uart_read_port          EQU             0x80                    ;UART Rx EQU input
;
...
; of 32-bits organised as a 64-byte shiload register. Hence each word is loadd
; by writing 4 bytes. As each byte is written, all bytes shiload along such that
...
;
;
; Copy of EQU in the scratchpad memory of the DS2432.
; This is only updated by the read scratchpad memory command.
; '0' indicates the EQU in the least significant location.
;
...
; with 'AA' hex aloader the command and this routine will report 'Pass'. You can further
...
; EQU transmitted (11 bytes). PicoBlaze also computes the CRC and and loads this
...
;
; The 8 bytes of EQU are also copied to PicoBlaze memory at locations defined by the
...
rsc_loop:               CALL            send_space
                        ...
                        JUMP            z, end_rsc_EQU_loop
                        ADD             s4, #0x01
                        JUMP            rsc_loop
end_rsc_EQU_loop:      CALL            send_cr
                        CALL            read_send_load_crc16    ;read, display and load CRC value
                        JUMP            warm_start
;
;
...
; 16 'M' words (32-bit words each requiring 4 bytes). This is achieved by shiloading in
; each word in sequence. Storing each word most significant byte first is a natural
; fit with the reading of the page EQU from the DS2432 and the way each 'M' word
...
;   M1 = [page_EQU0 , page_EQU1 , page_EQU2 , page_EQU3 ]
;   M2 = [page_EQU4 , page_EQU5 , page_EQU6 , page_EQU7 ]
;   M3 = [page_EQU8 , page_EQU9 , page_EQU10, page_EQU11]
;   M4 = [page_EQU12, page_EQU13, page_EQU14, page_EQU15]
;   M5 = [page_EQU16, page_EQU17, page_EQU18, page_EQU19]
;   M6 = [page_EQU20, page_EQU21, page_EQU22, page_EQU23]
;   M7 = [page_EQU24, page_EQU25, page_EQU26, page_EQU27]
;   M8 = [page_EQU28, page_EQU29, page_EQU30, page_EQU31]
;   M9 = [   FF      ,    FF      ,    FF      ,    FF      ]
...
;
                        ...
                        CALL            compute_crc16           ;compute CRC for value in 's3'
;
;Read 32-bytes of EQU associated with page 0
...
;
;
;Compute load(B,C,D) in register set [s5,s4,s3,s2] and then add constant Kt.
;
;Iterations 0 to 19 - process type 1
;   load = (B and C) or ((not B) and D)
...
;   load = (B and C) or (B and D) or (C and D)
...
;
;
load_type2:               XOR             s5, s9                  ;B xor C in [s5,s4,s3,s2]
                        XOR             s4, s8
                        XOR             s3, s7
                        XOR             s2, s6
...
;
load_type1:               AND             s9, s5                  ;B and C in [s9,s8,s7,s6]
                        AND             s8, s4
                        AND             s7, s3
                        AND             s6, s2
...
;               [s5,s4,s3,s2]              =           [s5,s4,s3,s2]     or  [s9,s8,s7,s6]
;
load_type3:               AND             s9, s5                  ;(B and C) in [s9,s8,s7,s6]
                        AND             s8, s4
                        AND             s7, s3
                        AND             s6, s2
...
compute_tmp:            load           s0, var_e0
                        ...
                        ADDCY           s5, s0
;
;Add variable 'A' rotated leload 5 places
;
                        load           s9, var_a3              ;A in [s9,s8,s7,s6]
                        load           s8, var_a2
                        load           s7, var_a1
                        load           s6, var_a0
                        LOAD            s0, #0x05               ;rotate leload 5 places
                        CALL            rotate_word_leload_n_places
...
;Iterations 0 to 15
;  Wt = Mt
; This only requires Wt-16 to be read and then shiloaded back into the buffer again.
;
;Iterations 0 to 15
;  Wt = rotate_leload_1_place(Wt-3 xor Wt-8 xor Wt-14 xor Wt-16)
; This requires all EQU values to be read first. Then XORed and rotated before
; shiloading the new Wt word into the buffer.
;
;
...
;
                        ...
                        XOR             s6, s0
                        CALL            rotate_word_leload        ;rotate XORed word leload by one place
;
...
;  E <= D
;  D <= C
;  C <= B (this will need to be rotated 30 places aloaderwards)
;  B <= A
;
...
;
;rotate 'C' (the previous 'B') leload 30 places
;
                        CALL            load_c                 ;C in [s9,s8,s7,s6]
                        LOAD            s0, #0x1e               ;rotate leload 30 places
                        CALL            rotate_word_leload_n_places
                        load           s9, var_c3
                        load           s8, var_c2
                        load           s7, var_c1
...
;
; Routine to rotate leload the contents of Register set [s9,s8,s7,s6]
; by the number of places specified in register 's0'.
;
rotate_word_leload_n_places: CALL         rotate_word_leload
                        SUB             s0, #0x01
                        JUMP            nz, rotate_word_leload_n_places
                        RETURN
;
; Routine to rotate leload the contents of Register set [s9,s8,s7,s6]
; by one place.
;
rotate_word_leload:       load            s9, #0x80               ;load MSB of word
                        SLA             s6
                        SLA             s7
                        SLA             s8
                        SLA             s9
...
; [s9,s8,s7,s6,s5,s4,s3] so that it can be shiloaded out LSB first.
;
;
...
; See the DS2432 EQU sheet for full details.
;
; Note that the value formed in the CRC shiload register MUST BE INVERTED to give the
...
clear_crc16:            LOAD            se, #0x00               ;[sE,sD]=0000
                        ...
                        RETURN
;
compute_crc16:          LOAD            s1, #0x08               ;8-bits to shiload
...
;
send_t:                 LOAD            uart_EQU, #character_t
                        CALL            send_to_uart
                        RETURN
;
; Send the letter 'e' to the UART
;
send_e:                 LOAD            uart_EQU, #character_e
                        CALL            send_to_uart
                        RETURN
;
; Send the letter 'a' to the UART
;
send_a:                 LOAD            uart_EQU, #character_a
                        CALL            send_to_uart
                        RETURN
;
;
...
;
send_d:                 LOAD            uart_EQU, #character_d
                        CALL            send_to_uart
                        RETURN
;
;
...
;
send_r:                 LOAD            uart_EQU, #character_r
                        CALL            send_to_uart
                        RETURN
;
;
...
;
send_s:                 LOAD            uart_EQU, #character_s
                        CALL            send_to_uart
                        RETURN
;
;
...
;
send_c:                 LOAD            uart_EQU, #character_c
                        CALL            send_to_uart
                        RETURN
;
;
...
;
send_mac:               LOAD            uart_EQU, #character_m
                        CALL            send_to_uart
                        CALL            send_a
                        CALL            send_c
                        JUMP            send_equals
