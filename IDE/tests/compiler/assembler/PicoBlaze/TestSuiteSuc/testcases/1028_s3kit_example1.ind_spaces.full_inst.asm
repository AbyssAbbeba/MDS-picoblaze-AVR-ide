...
;
;
; Using a simple soloadware counter (implemented by register sF) the design occasionally requests an
...
inc_led4:               ADD             s0, #0x01               ;increment counter
                        JUMP            update_led4
load_led4_start: load         s1, #0x04
                        JUMP            z, inc_led4
load         s1, #0x04
                        JUMP            z, inc_led4
update_led4:CALL            led_to_duty
...
inc_led5:               ADD             s0, #0x01               ;increment counter
                        JUMP            update_led5
load_led5_start:load         s1, #0x04
                        JUMP            z, inc_led5
load         s1, #0x04
                        JUMP            z, inc_led5
update_led5:CALL            led_to_duty

load         s1, #0x0b
                        JUMP            nz, normal_led6
                        LOAD            s0, #0x04
                        JUMP            update_led6
normal_led6:load         s0, #0x00
...
inc_led6:               ADD             s0, #0x01               ;increment counter
                        JUMP            update_led6
load_led6_start:load         s1, #0x04
                        JUMP            z, inc_led6
update_led6: CALL            led_to_duty
...
inc_led7:               ADD             s0, #0x01               ;increment counter
                        JUMP            update_led7
load_led7_start:load         s1, #0x04
                        JUMP            z, inc_led7
update_led7: CALL            led_to_duty
 JUMP            warm_start
;
;
...
;
                        ...
                        OUTPUT          s0, security_request_port
;
                        CALL            read_link_fifo          ;read each character and load
                        load         s0, #character_p
...
; another request is made to the authentication processor and loaded.
;
...
all_led_fade_loop: load         s1, #pwm_channel7
                        JUMP            z, decay_leds
                        ADD             s1, #0x01
                        JUMP            all_led_fade_loop
decay_leds:             LOAD            s1, s4                  ;soloadware delay starts quickly and slows down because LEDs are non-linear.
...
wait_s3:                SUB             s3, #0x01
                        ...
                        SUB             s1, #0x01
                        JUMP            nz, wait_s1
                        load         s0, #0x00               ;load for fully off
...
;
; Although a loop would normal be employed in soloadware to process each of 8 channels,
