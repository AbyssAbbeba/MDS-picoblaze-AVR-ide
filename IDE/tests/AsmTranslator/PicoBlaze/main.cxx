...
int initTestEnv()
{
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Register test suite: Mediatronix
    // -----------------------------------------------------------------------------------------------------------------

    // Add the suite to the CUnit test registry.
    CU_pSuite testSuiteMed = CU_add_suite ( "Mediatronix",
                                            TestMedAsmTrans::init,
                                            TestMedAsmTrans::clean );
    if ( NULL == testSuiteMed )
    {
    ...
    if ( false == TestMedAsmTrans::addTests(testSuiteMed) )
    {
        CU_cleanup_registry();
        return CU_get_error();
    }
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Register test suite: openPICIDE
    // -----------------------------------------------------------------------------------------------------------------

    // Add the suite to the CUnit test registry.
    CU_pSuite testSuiteOPI = CU_add_suite ( "openPICIDE",
                                            TestOpenPICIDEAsmTrans::init,
                                            TestOpenPICIDEAsmTrans::clean );
    if ( NULL == testSuiteOPI )
    {
        // Unable to add the test suite to the registry.
        CU_cleanup_registry();
        return CU_get_error();
    ...
    // Add unit tests to the suite.
    if ( false == TestOpenPICIDEAsmTrans::addTests(testSuiteOPI) )
    {
        CU_cleanup_registry();
        return CU_get_error();
    }
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Register test suite: Xilinx
    // -----------------------------------------------------------------------------------------------------------------

    // Add the suite to the CUnit test registry.
    CU_pSuite testSuiteXil = CU_add_suite ( "Xilinx",
                                            TestXilAsmTrans::init,
                                            TestXilAsmTrans::clean );
    if ( NULL == testSuiteXil )
    {
        // Unable to add the test suite to the registry.
        CU_cleanup_registry();
        return CU_get_error();
    ...
    // Add unit tests to the suite.
    if ( false == TestXilAsmTrans::addTests(testSuiteXil) )
    {
        CU_cleanup_registry();
        return CU_get_error();
    }
...
{
    std::cout << "Integration testing environment for the MDS assembler translator tool." << std::endl
