 ...
 * @file TestOpenPICIDEAsmTrans.cxx
 */
// =============================================================================

#include "TestOpenPICIDEAsmTrans.h"
...
int TestOpenPICIDEAsmTrans::clean()
{
    delete m_compiler;
    delete m_options;
    delete m_msgInt;
...
bool TestOpenPICIDEAsmTrans::addTests ( CU_pSuite suite )
{
    using namespace boost::filesystem;

    std::vector<std::string> testCaseFiles;

    for ( directory_iterator dir ( path("openPICIDE") / "testcases" );
...
void TestOpenPICIDEAsmTrans::testFunction()
{
    using namespace boost::filesystem;
    ...
    m_translator->clear();
    bool result = m_translator->translate ( AsmTranslator::V_KCPSM_OPENPICIDE,
                                            ( path("openPICIDE") / "results"   / (testName + ".asm") ).string(),
                                            ( path("openPICIDE") / "testcases" / (testName + ".psm") ).string(),
                                            true );

    {
        std::ofstream logFile( ( path("openPICIDE") / "results" / (testName + ".log") ).string() );
