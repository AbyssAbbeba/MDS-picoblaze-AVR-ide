...
SimPortLogger::SimPortLogger(QWidget *parent, MCUSimControl *controlUnit) :
    QWidget(parent)
{
    ui.setupUi(this);
...
void SimPortLogger::DecoderChanged(int /*o*/)
{
    ValueChanged();
}

void SimPortLogger::handleChange(int /*o*/)
{
    ValueChanged();
}

void SimPortLogger::deviceChanged()  //8
{
...
void SimPortLogger::deviceReset()  //8
{
    this->value = m_plio->getOutputArray()[this->address];
    ValueChanged();
}

void SimPortLogger::handleUpdateRequest(int mask) //8
{
    if ( 4 & mask)
    {
        this->value = m_plio->getOutputArray()[this->address];
...
void SimPortLogger::handleEvent(int subsysId, int eventId, int /*locationOrReason*/, int /*detail*/) //8
{
...
void SimPortLogger::setReadOnly(bool /*readOnly*/)
{
}

// main loop, event
void SimPortLogger::ValueChanged()
{
...
void SimPortLogger::addrChanged()
{
    address = ui.lineEditAddress->text().toUInt(0,10);
...
void SimPortLogger::DisplayNumber(unsigned char Numero)
{
    if ( Numero & bit0)
    {
        flag_0 = false;
...
void SimPortLogger::ValueDecode(void)
{
    // Bin to BCD
...
void SimPortLogger::paintEvent(QPaintEvent */*e*/)
{
...
void SimPortLogger::CreateItems(void)
{
