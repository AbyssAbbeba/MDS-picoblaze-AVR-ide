...
/** Define level of input data checking
 *
 * Possible values are:
 *
...
void SrecFile::clearAndLoad(const char * filename) throw(DataFile::DataFileException)
{
	clearAndLoad(std::string(filename));
}

void SrecFile::clearAndLoad(const std::string & filename) throw(DataFile::DataFileException)
{
...
void SrecFile::save(const char * filename, bool makeBackup) throw(DataFile::DataFileException)
{
	save(std::string(filename), makeBackup);
}

void SrecFile::save(const std::string & filename, bool makeBackup) throw(DataFile::DataFileException)
{
	// Create backup file
	if ( true == makeBackup ) {
	...
	if (
		( false == m_blockHeader.m_mname.empty() )
			||
...
inline void SrecFile::writeBlockHeaderSRecord (
	std::fstream & file,
...
inline void SrecFile::writeRecordCountSRecord(std::fstream & file, int recordCount) const
{
	std::string record;
	char tmp[5];
...
inline bool SrecFile::writeDataSRecord(std::fstream & file, unsigned int address, const std::string & dataField) const
{
	int addrLength;
...
inline void SrecFile::writeEndOfBlockSRecord(std::fstream & file, unsigned int sea) const
{
	char tmp[9];
	std::string record;
...
inline int SrecFile::hex2int(const std::string & source) const {
	...
	for ( int j = 0, i = (source.size() - 1); i >= 0; j++, i-- ) {
		int tmp = 0;
		switch ( source[i] ) {
			case '0': tmp = 0; break;
			case '1': tmp = 1; break;
...
inline std::string SrecFile::vec2str(const std::vector<unsigned char> & source) const {
	...
	for (
		std::vector<unsigned char>::const_iterator i = source.begin();
		i != source.end();
		i++
	) {
		result.push_back(*i);
