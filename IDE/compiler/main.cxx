...
void printHelp ( const char * executable )
{
    ...
    std::cout << QObject::tr("Examples:").toStdString() << std::endl
              << QObject::tr("  %1 --arch=PicoBlaze --plang=asm --hex=abc.hex abc.asm")
                            .arg(executable).toStdString() << std::endl
              << QObject::tr("      Compile source code file `abc.asm' (--file=abc.asm) for architecture PicoBlaze")
                            .toStdString() << std::endl
              << QObject::tr("      (--arch=PicoBlaze) written in assembly language (--plang=asm), and create file")
                            .toStdString() << std::endl
              << QObject::tr("      `abc.hex' containing machine code generated generated by the compiler.")
                            .toStdString() << std::endl
              << QObject::tr("  %1 --plang asm --arch PicoBlaze --hex abc.hex abc.asm")
                            .arg(executable).toStdString() << std::endl
              ...
              << QObject::tr("  %1 -p asm -a PicoBlaze -x abc.hex abc.asm").arg(executable).toStdString() << std::endl
