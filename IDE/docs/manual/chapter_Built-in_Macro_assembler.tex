...
\section{Basics}
    ...
    \subsection{Symbols}
        ...
        \verb'    my_label:                          ; Program memory address, initialized by address counter.'

        \subsubsection{Special Symbols}
            MDS assembler supports only one special symbol, this symbol is always automatically defined on every line containing an instruction (or the \texttt{DB} directive, \ref{DB}), it is symbol ``\texttt{\$}''. ``\texttt{\$}'' contains address in the program memory where ``this instruction'' is going to be placed by the assembler.
...
\section{Compiler messages}
    ...
    \begin{description}
        \item[Unable to open file]~\\
            The given file cannot be opened, probably does not exist or your operating system refuses to grad the access to it. Check whether the file exists, and check your permissions.
        \item[Unable to write to file]~\\
            It is not possible to write into the given file. This is in most cases caused by wrong permissions set on the file or directory, or nonexistent directory in the file path.
        ...
        \item[User defined memory limit for X exceeded]~\\
            You have exceeded boundary of X memory defined by the \texttt{LIMIT} directive.
        \item[Instruction X requires operand Y to be of type(s) Z]~\\
            This means for example that you are trying to use a symbol defined as port address in place of register address.
        ...
        \item[Limit value X is not valid]~\\
            Invalid limit specifier for the \texttt{LIMIT} directive, only \texttt{D}, \texttt{R}, and \texttt{C} are valid.
        ...
        \item[Maximum macro expansion level X reached]~\\
            You can change your compiler settings and set higher expansion level. By default, maximum macro expansion level is unlimited, which is possibly dangerous and users should set some limit for them selfs.
        ...
        \item[Symbol already defined]~\\
            The given symbol has already been defined, there cannot coexist two or more symbols with the same name in one compilation unit.
