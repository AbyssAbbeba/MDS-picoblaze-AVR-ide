...
bool McuDeviceSpecAVR8::setupSimulator(AVR8Config & mcuConfig) {
	...
	for ( int i = 0; i < AVR8InterruptController::INTVEC__MAX__; i++ ) {
		...
		if (
			( true == m_interruptController.m_possibleInterrupts[AVR8InterruptController::INTVEC_INT0] )
				||
		...
		if (
			( true == m_interruptController.m_possibleInterrupts[AVR8InterruptController::INTVEC_TIMER2_COMP] )
				||
		...
		if (
			( true == m_interruptController.m_possibleInterrupts[AVR8InterruptController::INTVEC_USART_RXC] )
				||
