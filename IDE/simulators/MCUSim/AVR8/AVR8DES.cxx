...
inline OutputType AVR8DES::permute(InputType input, const InputType * permutationTable) {
	OutputType output = 0;
...
//                          │ 32b result  │
//                          ╰─────────────╯
inline uint32_t AVR8DES::feistelFunction(uint32_t halfBlock, uint64_t subkey) {
...
inline uint32_t AVR8DES::rotateLeft32(uint32_t input, unsigned int by) {
	by %= 32;
	return (input << by) | (input >> (32 - by));
}

inline uint32_t AVR8DES::rotateRight32(uint32_t input, unsigned int by) {
	by %= 32;
	return (input >> by) | (input << (32 - by));
}
...
//         ╰───────────────┴─→╢ Permuted choice 2 ╟──→ Subkey K₁₆ (48b)
//                            ╚═══════════════════╝
inline uint64_t AVR8DES::keySchedule(uint64_t & keyBlock, unsigned int round, bool decipher) {
	uint64_t subkey;
