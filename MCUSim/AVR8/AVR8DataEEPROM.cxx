...
inline void AVR8DataEEPROM::writeByte(float timeStep, unsigned int eecr) {
	using namespace AVR8RegNames;
...
unsigned int AVR8DataEEPROM::timeStep(float timeStep, unsigned int clockCycles) {
	using namespace AVR8RegNames;
...
MCUSim::RetCode AVR8DataEEPROM::directRead(unsigned int addr, unsigned int & data) const {
...
MCUSim::RetCode AVR8DataEEPROM::directWrite(unsigned int addr, unsigned int data) {
	if ( addr >= m_config.m_size ) {
		return MCUSim::RC_ADDR_OUT_OF_RANGE;
	}
